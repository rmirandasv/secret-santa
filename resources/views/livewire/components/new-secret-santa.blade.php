  ...
  <form wire:submit="save">
    ...
    <div x-cloak x-show="step === 1" class="p-4 flex flex-col justify-between">
      <div class="flex flex-col space-y-2">
        <template x-for="(participant, index) in participants" :key="index">
          <input type="text"
            class="p-2 block w-full mt-1 text-gray-600 border border-gray-300 rounded-md focus:outline-purple-500 focus:ring-purple-500 focus:border-purple-500 sm:text-sm"
            x-model="participant.name" x-bind:placeholder="`Name ${index+1}`" x-on:keydown.enter="checkParticipants"
            x-on:keydown.backspace="removeParticipant(index)">
        </template>
      </div>
      <div class="mt-4">
        <button type="button" class="py-1 px-2 bg-white border border-purple-400 rounded-md" x-on:click="addParticipant">
      ...
      <div class="mt-10">
        <button type="button" class="w-full px-4 py-2 text-sm font-medium text-white bg-purple-500 rounded-md disabled:opacity-50"
    ...
    <div x-cloak x-show="step === 2" class="p-4">
      <div class="flex flex-col space-y-2">
        ...
        <div class="flex flex-col">
          ...
          <textarea id="message" name="message" x-model="message"
            class="p-2 block w-full mt-1 text-gray-600 border border-gray-300 rounded-md focus:outline-purple-500 focus:ring-purple-500 focus:border-purple-500 sm:text-sm"></textarea>
      ...
      <div class="mt-10 flex items-center space-x-4">
        ...
        <button type="submit" class="w-full px-4 py-2 text-sm font-medium text-white bg-purple-500 rounded-md disabled:opacity-50"
